...
$doubleData = json_decode(file_get_contents('https://cdn.drifter.surf/content/rooms/double.json'), true);
...
$alpineData = [
    "selectedImage" => $doubleData["images"][$doubleData["default_room_type"]][0],
    "images" => $doubleData["images"],
    "checkIn" => "new Date(Date.now() + 86400000).toISOString().split('T')[0]",
    "checkOut" => "new Date(Date.now() + 2 * 86400000).toISOString().split('T')[0]",
    "rooms" => 1,
    "roomType" => $doubleData["default_room_type"],
    "prices" => $doubleData["prices"],
    "init" => "function() {
        const params = new URLSearchParams(window.location.search);
        this.roomType = params.get('type') || '" . $doubleData["default_room_type"] . "';
        this.rooms = parseInt(params.get('rooms')) || 1;
    }",
    "formatWhatsAppMessage" => "function() {
        return 'Hey there, I want to book ' + this.rooms + ' room' + (this.rooms > 1 ? 's' : '') + ' in the ' + this.roomType + ' from ' + this.checkIn + ' to ' + this.checkOut + ' please';
    }"
];
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo htmlspecialchars($doubleData['title']); ?> - Drifter Surf Hostel</title>
    ...
    <script>
        function handleBooking() {
            ...
            let  message = `Hey there, I want to book ${beds} bed${beds > 1 ? 's' : ''} in the ${roomType} room `;
...
<body class="bg-drifter-soft-beige text-drifter-brown" x-data='<?php echo safeJsonEncode($alpineData); ?>' x-init="init">
    ...
    <div class="container mx-auto px-4 pt-24 pb-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <template x-for="image in images[roomType]">
        ...
        <div class="mt-12 grid md:grid-cols-2 gap-12">
            <div>
                <h1 class="text-4xl font-bold mb-6"><?php echo htmlspecialchars($doubleData['title']); ?></h1>
                <p class="text-2xl font-bold text-drifter-light-brown mb-4">From $<span x-text="prices[roomType]"></span>/night</p>
                <p class="mb-6"><?php echo htmlspecialchars($doubleData['description']); ?></p>

                <!-- Room Features -->
                <div class="mb-8">
                    <h2 class="text-2xl font-bold mb-4">Room Features</h2>
                    <ul class="grid grid-cols-2 gap-4">
                        <?php foreach ($doubleData['features'] as $feature): ?>
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        <?php foreach ($doubleData['policies'] as $policy): ?>
            ...
            <div class="bg-white p-8 rounded-lg shadow-lg">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center">
                                <input type="radio" x-model="roomType" value="budget-fan" class="mr-2 accent-drifter-orange-brown">
                                Budget Double Room with Fan ($22/night)
                            </label>
                            <label class="flex items-center">
                                <input type="radio" x-model="roomType" value="standard-fan" class="mr-2 accent-drifter-orange-brown">
                                Standard Double Room with Fan ($24/night)
                            </label>
                            <label class="flex items-center">
                                <input type="radio" x-model="roomType" value="balcony-fan" class="mr-2 accent-drifter-orange-brown">
                                Balcony Double Room with Fan ($27/night)
                            </label>
                            <label class="flex items-center">
                                <input type="radio" x-model="roomType" value="balcony-ac" class="mr-2 accent-drifter-orange-brown">
                                Balcony Double Room with AC ($32/night)
                            </label>
                        </div>
                    </div>
                ...
                <div class="mb-6 text-xl font-bold">
                    Total: $<span x-text="rooms * prices[roomType]"></span> per night
                </div>
